...
const backButton = () => {
  ...
  document.getElementById("default-countries").style.display="flex";
    ...
    return (
      <>
        <div className="d-flex flex-column content">
          ...
          <div id="filter-div" className="filter-div m-3 p-3 rounded flex-row">
          ...
          <div id="back-div" className="back-div m-3 p-3 rounded">
          <FontAwesomeIcon onClick={backButton} className="mx-2 mt-1" icon={faBackward} />
            Back
          </div>

          <div id="regions-div" className="regions-div mx-3 p-2 position-absolute rounded">
            <p className="region-p" onClick={() => filterByRegion("Africa")}>Africa</p>
            <p className="region-p" onClick={() => filterByRegion("Americas")}>Americas</p>
            <p className="region-p" onClick={() => filterByRegion("Asia")}>Asia</p>
            <p className="region-p" onClick={() => filterByRegion("Europe")}>Europe</p>
            <p className="region-p" onClick={() => filterByRegion("Oceania")}>Oceania</p>
          </div>

            <ul className="default-countries" id="default-countries">
            {defaultCountries.map((country) => (
          <li
            className="rounded list-item list-group-item mt-5"
          ...
          >
            ...
            <ul className="searched country" id="searched-country">
            {searchedCountry.map((country) => (
          <li
            className="country-li rounded list-item list-group-item mt-5"
            key={country.alpha3Code}
          >
            <img
              className="logo-img2"
            ...
            <p className="country-name h1">{country.name? country.name : "Unknown Name"}</p>
            <p className="native-name"><span className="fw-bold">Native Name:</span> {country.nativeName? country.nativeName : "Unknown Native Name"}</p>
            ...
            <div className="border-div d-flex flex-column flex-wrap">
            <p><span className="fw-bold">Border Countries:</span></p>
            {country.borders?.join(', ') || 'No Border country'}
            </div>
            </li>
             ))}
            </ul>
